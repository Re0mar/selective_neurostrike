...
def load_datasets(malicious_only=False):
    ...
    # Labels: "fraud", "hate", "smuggle", "self", "jailbreak", "sexual", "violent", "cyber"
def load_sorted_datasets(malicious_only=False, label="none", return_dataframe=False):
    all_texts = []
    all_labels = []
    ...
    if label == "fraud":
        ds = pd.read_csv('../outputs/fraud_and_deception.csv')
    elif label == "hate":
        ds = pd.read_csv('../outputs/hate_or_harassment.csv')
    elif label == "smuggle":
        ds = pd.read_csv('../outputs/illegal_goods_or_services.csv.csv')
    elif label == "self":
        ds = pd.read_csv('../outputs/self_harm.csv.csv')
    elif label == "jailbreak":
        ds = pd.read_csv('../outputs/policy_evasion_or_jailbreak.csv.csv')
    elif label == "sexual":
        ds = pd.read_csv('../outputs/sexual_harm.csv.csv')
    elif label == "violent":
        ds = pd.read_csv('../outputs/violent_physical_harm.csv.csv')
    elif label == "cyber":
        ds = pd.read_csv('../outputs/cybercrime.csv.csv')

    if label != "none":
        for prompt, categories in zip(ds['text'], ds['category']):
            if label in categories:
                all_texts.append(prompt)
                all_labels.append(1)

    print(f'Number of prompts: {len(all_texts)}, selected label: {label}')
